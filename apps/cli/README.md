...
Production runs require built package and frontend artifacts. From the repository root, run `pnpm run build` separately, then use `pnpm dsh <args...>` to run the same compiled bin the npm package exposes and forward every argument. The [checkout-execution reference](reference/README.md#checkout-execution) owns the built and explicit source-launch paths.
