...
From the repository root, run `pnpm run build` after a fresh checkout and whenever artifacts need updating, then use `pnpm dsh <args...>`. The root script runs `apps/cli/lib/bin.js` under plain Node, exactly like the installed package, and forwards every argument without rebuilding. Missing or stale package, frontend, or Client plugin artifacts therefore require an explicit rebuild. The process inherits the launch environment; set `NODE_USE_ENV_PROXY=1` when a supporting Node version must honor `HTTP_PROXY` and `HTTPS_PROXY`.

`pnpm dsh:source <args...>` is the explicit contributor-only source path. It launches `apps/cli/src/bin.ts` with `node --import tsx/esm` without building; generated Host, frontend, and Client plugin artifacts remain separate inputs and may still require `pnpm run build`.
