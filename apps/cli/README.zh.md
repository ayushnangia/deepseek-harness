...
生产运行需要已构建的包与前端产物。请在仓库根目录单独运行 `pnpm run build`，然后使用 `pnpm dsh <args...>` 运行 npm 包所暴露的同一个已编译 bin，并转发所有参数。已构建与显式源码启动路径以 [checkout 执行参考](reference/README.md#checkout-execution)为准。
