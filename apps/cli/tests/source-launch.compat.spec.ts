 ...
 * with the exact explicit source vector (`node --import tsx/esm`, the
 * vector the explicit `dsh:source` script invokes) and assert the
 ...
 * breaks this gate instead of contributors' `pnpm dsh:source`; the built-bin
...
describe('dsh SOURCE launcher (node --import tsx/esm)', () => {
  it('launches the source CLI without building', async () => {
    ...
    expect(rootPackage.scripts?.dsh).toBe('dsh')
