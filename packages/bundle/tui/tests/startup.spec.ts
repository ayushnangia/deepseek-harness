 ...
 * tree: the joined prompt positional becomes injected runner config, an empty
 * invocation validly starts with no first prompt, and help leaves the runner
 * pending.
 */
...
describe('tui command-line provider', () => {
  it('joins the prompt positional into the runner config', async () => {
    ...
    expect(startup).toEqual({ prompt: 'run the tests' })
    expect(observed.runnerConfig).toEqual({ prompt: 'run the tests' })
    expect(observed.exits).toEqual([])
  })

  it.each([{ args: [] }, { args: ['   '] }])('starts the REPL with no first prompt on a bare invocation ($args)', async ({ args }) => {
    const { startup, observed } = await bootStartup(args)
    expect(startup).toEqual({ prompt: '' })
    expect(observed.runnerConfig).toEqual({ prompt: '' })
    expect(observed.exits).toEqual([])
  })
