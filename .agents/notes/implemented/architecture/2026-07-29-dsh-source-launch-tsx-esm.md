...
The node-compat CI matrix (Node 22.19 and 26) runs `dsh-source-launch-smoke` (`apps/cli/tests/source-launch.compat.spec.ts`): a keyless piped-stdio launch of the exact explicit source vector asserting the non-zero-exit TTY refusal. Any future Node change to module hooks or TypeScript handling turns this gate red instead of breaking contributors' `pnpm dsh:source`.
