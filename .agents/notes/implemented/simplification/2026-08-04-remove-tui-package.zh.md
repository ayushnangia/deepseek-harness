...
该包还会使仓库所支持的应用清单产生误导。当时可运行产品使用 Web、ACP（Agent Client Protocol）、JSON-RPC 或一次性 CLI（命令行界面）入口，SDK 却仍提供终端选项，而没有任何示例或产品命令会实际使用它。

## 决策

删除当时位于 `packages/` 下 `ui/tui` 的 TUI 包，不提供兼容包或别名。其源码、包测试、终端快照、依赖声明、已打补丁的 `pi-tui` 产物、workspace 引用、生成的服务目录条目和文档会一并移除。通用宿主能力与 agent loop（智能体循环）能力保持不变。
...
仓库搜索结果与生成的服务目录中不再包含已删除包的任何残留：没有依赖补丁、服务键或指向已移除路径的链接。常规源码构建、类型检查、lint、hygiene、文档门禁以及其余组装快照测试套件均可在没有已删除 workspace 的情况下运行。
...
依赖已删除包的 import 和 `cordis.yml` 条目会直接失败，不会得到兼容转换。在 [tui 组合包决策](../feature/2026-08-13-tui-bundle.md)以独立的 `packages/bundle/tui` 包重新引入终端前端之前，Web 一直是唯一已交付的交互界面；当前终端前端由该记录持有，而本记录仍是旧包移除决策的依据。

提供方无关的命令、用户交互、审批、工具呈现、PTY 与会话投影能力仍可供其他宿主使用。重新引入终端前端时，必须为其提供具名产品或部署、显式包边界、具体交互提供方，以及组装后的生命周期与 transcript（文本记录）验收；tui 组合包在其自身记录中对照这些条件进行衡量。
