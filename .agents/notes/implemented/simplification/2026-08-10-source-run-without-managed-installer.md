...
The repository supports checkout execution through its root `pnpm` scripts. The `dsh` entry runs the built package bin under the [built-checkout decision](../process/2026-08-14-default-checkout-built-cli.md), while `dsh:source` explicitly launches `apps/cli/src/bin.ts` through `node --import tsx/esm`; artifact generation is the separate `pnpm run build` operation defined by the [source-launch/build separation decision](2026-08-12-separate-source-launch-from-build.md). Both scripts forward arguments and inherit the caller's environment, including `NODE_USE_ENV_PROXY=1` when a supporting Node version must honor `HTTP_PROXY` and `HTTPS_PROXY`. Users select Web with `pnpm dsh web` and headless execution with `pnpm dsh --profile headless "task"`. The independent ACP example remains available through `pnpm run demo:acp`.
...
Verification covers repository-wide references to the removed entry points, documentation links, generated third-party-notice freshness, both `package.json` command mappings, and a source CLI smoke through the exact `node --import tsx/esm` runtime vector.
