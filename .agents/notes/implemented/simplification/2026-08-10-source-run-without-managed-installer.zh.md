...
仓库通过根目录的 `pnpm` 脚本支持 checkout 执行。`dsh` 项按[已构建 checkout 决策](../process/2026-08-14-default-checkout-built-cli.md)运行已构建的包 bin，`dsh:source` 则通过 `node --import tsx/esm` 显式启动 `apps/cli/src/bin.ts`；产物生成是独立的 `pnpm run build` 操作，由[源码启动与构建分离决策](2026-08-12-separate-source-launch-from-build.md)规定。两个脚本都会转发参数并继承调用方环境；当支持环境代理的 Node 版本必须遵循 `HTTP_PROXY` 和 `HTTPS_PROXY` 时，调用方可设置 `NODE_USE_ENV_PROXY=1`。用户使用 `pnpm dsh web` 选择 Web，使用 `pnpm dsh --profile headless "task"` 选择无头执行。独立的 ACP（Agent Client Protocol）示例仍可通过 `pnpm run demo:acp` 运行。
...
验证范围包括仓库内对已移除入口点的所有引用、文档链接、生成的第三方声明文件的新鲜度、`package.json` 中两个命令的映射，以及通过准确的 `node --import tsx/esm` 运行方式对源码 CLI 进行的冒烟测试。
