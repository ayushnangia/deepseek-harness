...
The package also made the repository's supported application inventory misleading. Runnable products at the time used Web, ACP, JSON-RPC, or one-shot CLI entry points, while the SDK continued to offer a terminal choice that no example or product command exercised.

## Decision

The TUI package, then at `ui/tui` under `packages/`, is deleted without a compatibility package or alias. Its source, package tests, terminal snapshots, dependency declarations, patched `pi-tui` artifact, workspace references, generated service catalog entry, and documentation are removed together. Generic host and agent-loop capabilities remain unchanged.
...
Repository searches and generated catalogs contain no remnant of the deleted package: no dependency patch, service key, or link to the removed path. The ordinary source build, typecheck, lint, hygiene, documentation gates, and remaining assembled snapshot suites run without the deleted workspace.
...
Imports and `cordis.yml` rows that depended on the deleted package fail instead of being translated. Web remained the only shipped interactive surface until the [tui bundle decision](../feature/2026-08-13-tui-bundle.md) reintroduced a terminal frontend as the separate `packages/bundle/tui` package; that note owns the current terminal frontend, and this note remains the authority for the old package's removal.

The provider-neutral command, user-questions, approval, tool-presentation, PTY, and session-projection capabilities remain available to other hosts. Reintroducing a terminal frontend requires a named product or deployment, an explicit package boundary, a concrete interaction provider, and assembled lifecycle and transcript acceptance for that frontend; the tui bundle is measured against those conditions in its own note.
