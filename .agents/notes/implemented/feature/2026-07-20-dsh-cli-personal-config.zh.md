...
**`dsh` CLI（命令行界面；`apps/cli`，npm 名 `@deepseek-ai/dsh`）。** `apps/*` 是位于 `packages/*` 库之上的产品组装层。一个 bin 负责分发默认交互式 TUI、`-p`/`--prompt` 无头轮次和 `web` 界面。TUI 以调用目录为 workspace，启动 `examples/tui-agent/cordis.yml`（或 `--config` 指定的配置）。在 checkout 中，根目录的 `pnpm dsh` 脚本运行已构建的包 bin，但不会重新构建；`pnpm dsh:source` 是显式的 tsx 源码向量。[已构建 checkout 决策](../process/2026-08-14-default-checkout-built-cli.md)规定默认路径，[源码启动决策](../architecture/2026-07-29-dsh-source-launch-tsx-esm.md)规定源码向量，[源码启动与构建分离决策](../simplification/2026-08-12-separate-source-launch-from-build.md)规定产物生成。
